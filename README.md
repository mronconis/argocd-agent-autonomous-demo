...
```bash
kubectl get secret argocd-initial-admin-secret -o "jsonpath={.data.password}" -n $NAMESPACE_NAME --context kind-$PRINCIPAL_CLUSTER_NAME | base64 -d && echo
```

Open your browser at https://localhost:8443, use `admin` as your username and as your password the one read by the secret.

### Test Application Synchronization (Autonomus Mode)
